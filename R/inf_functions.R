...
#' @title boot_reg
#'
#' @description Bootstrap regression coefficients in order to estimate their sampling distribution.
#'
#' @param output An object of class nmf_output
#'
...
boot_reg_stratified = function(output, samples, parallel = 4,
  ...
  for( group in 1:nrow(categorical_groups)){
    group_count[group] <- sum(prodlim::row.match(covariate_block,categorical_groups[group,],0))
  }
  ...
  if(parallel ==1){
    for(i in 1:samples){
      ...
      for(group_strat in group_levels){
        ...
        sampled_indices <- which(prodlim::row.match(covariate_block,categorical_groups[group_strat,],0)==1)[
    ...
    #parallel loop start
    to_return<- foreach::foreach(i=1:samples, .export = c("get_regression_coefs"),
...
boot_plot = function(boot_samples,
  ...
  if( is.null(newdata)){
    ...
    if(any(class(boot_reg_list[[topic]])=="betareg")){
      ...
      ##### create and evaluate plot
      topic_plot <- ggplot2::ggplot(data = boot_effect, ggplot2::aes(x=weight, y=covar, height = ggplot2::after_stat(density)))+
        ggridges::stat_density_ridges(fill = "lightblue",rel_min_height = 0.005,scale = 1.25)+
