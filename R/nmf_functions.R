#' @title solve_nmf
#'
#' @description Solve the non-negative matrix factorization problem with specified inputs.
#'
...
solve_nmf = function(input, user_anchors = NULL){
  ...
  if(!(is.null(user_anchors))){
    ...
    #qr does the full Nvocab decomp.  This pivots any user selected anchors up to the front.
...
#' @title get_lambda
#'
#' @description Extracts the inverse lambdas and matches them to the anchor words.
#'
#' @param output An object of class nmf_output.
#'
