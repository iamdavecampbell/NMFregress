#' @title solve_nmf
#'
#' @description Solve the non-negative matrix factorization problem with
#' specified inputs.
#'
#' @param input An object of class nmf_input.
#'
#' @param user_anchors User-specified anchors. These can be specified if the
...
#' words,
#' the order the anchor words were found, the entire vocabulary (sorted to match
#'  the word-topic
#' matrix), and the number of topics. These are all needed to print a summary of
#'  the topics.
#'
...
solve_nmf <- function(input, user_anchors = NULL) {
  ...
  ##### find anchors w/ qr decomposition
  # (using Gaussian random projection if specified)
  ##### store anchors
  if (!(is.null(user_anchors))) {
...
#'
#' @return A list with one element for each topic. These are labeled by anchor
#' words. Each
#' list element contains the top words in that topic, printed in order of
#' importance (largest
...
#'                         covariates = acts)
#' my_output = solve_nmf(my_input)
#' print_top_words(my_output, n = 25)
#'
#' @export
...
#' @title get_lambda
#'
#' @description Extracts the inverse lambdas and matches them to the
#' anchor words.
#'
#' @param output An object of class nmf_output.
#'
...
#' @title get_reconstruction_error.
#'
#' @description Calculates the goodness of fit measure for matrix reconstruction
#' The Frobenius norm is computed based on a TDM reconstructed with
#' incrementally increasing topics
#'
#' @param output An object of class nmf_output from solve_nmf
#'
#' @param input all of the information passed into the original
#' solve_nmf function
#'
#' @param n_topics optional, the maximum number of topics to consider
#'
#' @return a data frame with columns 'topics' taking values from 0: n_topics.
#' Note that zero is actually the Frobenius norm of the original TDM.
#' When 'topics'>0 the norm is applied to
#' "reconstruction(based on # topics) - original"
#'
#' @import ggplot2
#'
#' @examples
...
get_reconstruction_error <- function(output,
  ...
  # n_topics= 1. handle differently since indexing breaks the matrix and
  ...
  p <- frobeniusnorm |>
    ggplot2::ggplot(ggplot2::aes(x = topics, y = frobenius_norm)) +
    ggplot2::geom_point()
  print(p)
