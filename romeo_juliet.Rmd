...
Sometimes it is helpful to see how additional topics assist with the reconstruction error of the original TDM.  There is now a way to do that.  This function calculates the Frobenius norm of reconstruction, $||X- \phi_t\theta_t||$, based on $t\in {1,2,...T}$ topics.  In the output, the topic value of  = 0 implies that no reconstruction is performed and instead the norm is applied to the original TDM $||X||$.

```{r}

reconstruction_error = get_reconstruction_error(my_output, my_input)

reconstruction_error$frobeniusnorm
```
